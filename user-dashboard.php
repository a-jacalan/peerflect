...
$sql = "SELECT isVerified, userType FROM Users WHERE userid = ?";
if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("i", $param_id);
    $param_id = $_SESSION["id"];
    if ($stmt->execute()) {
        $stmt->bind_result($isVerified, $userType);
        if ($stmt->fetch()) {
            if ($isVerified == 0) {
...
$username = $email = $fullname = $profile_image = $school = $user_type = "";

// Prepare a select statement
$sql = "SELECT username, email, CONCAT(firstname, ' ', lastname) AS fullname, profileimageurl, school, userType FROM Users WHERE userid = ?";
...
if ($stmt = $conn->prepare($sql)) {
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            $stmt->bind_result($username, $email, $fullname, $profile_image, $school, $userType);
            if ($stmt->fetch()) {
            }
        }
    }
    ...
    <div class="user-profile">
        ...
        <div>
            ...
            <div class="contributor-status">
                <p><strong>User Type:</strong> Regular</p>
            </div>
            <?php elseif($userType == 'contributor'): ?>
            <div class="contributor-status">
